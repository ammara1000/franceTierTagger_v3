...
public abstract class MixinPlayerEntity implements TierModifier {
    ...
    public Text lol(Text original) {
        ...
        if (suffix!=null) {
            return original.copy().append(Text.literal(sufix).styled(s -> s.withColor(Formatting.WHITE)).styled(s -> s.withFont(new StyleSpriteSource.Font(Identifier.of("frtl","lol")))));
        }
        return original;
    }
}
